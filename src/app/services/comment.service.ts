...
import { CommentPage, CommentRequest, CommentResponse, LikerResponse } from '../models/comment.interface';
...
export class CommentService {
  ...
  /** One page of a post's TOP-LEVEL comments, newest first -- page 0 (default) is the most recent `size` (default 10). */
  ...
  /** One page of a comment's direct replies, oldest first. */
  getReplies(commentId: number, page = 0, size = 10): Observable<ApiResponse<CommentPage>> {
    return this.httpClient.get<ApiResponse<CommentPage>>(this.url + `/comment/${commentId}/replies`, {
      params: { page, size },
    });
  }
  ...
  getCommentLikes(id: number): Observable<ApiResponse<LikerResponse[]>> {
    return this.httpClient.get<ApiResponse<LikerResponse[]>>(this.url + `/comment/${id}/likes`);
  }
}
