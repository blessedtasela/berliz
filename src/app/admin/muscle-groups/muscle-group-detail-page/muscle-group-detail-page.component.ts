import { DatePipe } from '@angular/common';
import { Component, OnDestroy, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { Store } from '@ngrx/store';
import { Subject, takeUntil } from 'rxjs';
import { MuscleGroups } from 'src/app/models/muscle-groups.interface';
import { loadMuscleGroups } from 'src/app/state/muscle-group/muscle-group.actions';
import { selectMuscleGroups } from 'src/app/state/muscle-group/muscle-group.selectors';

/**
 * Routed detail page for a single muscle group —
 * `/dashboard/muscle-groups/:id` (also reachable via
 * `/dashboard/hub/muscle-groups/:id`, same lazy module).
 *
 * Replaces the old MuscleGroupDetailsModalComponent dialog. Looks the
 * muscle group up in the already-loaded `muscleGroups` slice and dispatches
 * `loadMuscleGroups()` if it is empty (direct link / refresh).
 */
@Component({
  selector: 'app-muscle-group-detail-page',
  templateUrl: './muscle-group-detail-page.component.html',
  styleUrls: ['./muscle-group-detail-page.component.css']
})
export class MuscleGroupDetailPageComponent implements OnInit, OnDestroy {
  muscleGroupData: MuscleGroups | null = null;
  loading = true;

  private destroy$ = new Subject<void>();

  constructor(
    private route: ActivatedRoute,
    private store: Store,
    private datePipe: DatePipe) {
  }

  ngOnInit(): void {
    this.route.paramMap
      .pipe(takeUntil(this.destroy$))
      .subscribe(params => {
        const id = Number(params.get('id'));
        if (!id || Number.isNaN(id)) {
          this.loading = false;
          this.muscleGroupData = null;
          return;
        }
        this.loadMuscleGroup(id);
      });
  }

  ngOnDestroy(): void {
    this.destroy$.next();
    this.destroy$.complete();
  }

  private loadMuscleGroup(id: number): void {
    this.loading = true;
    let dispatched = false;
    this.store.select(selectMuscleGroups)
      .pipe(takeUntil(this.destroy$))
      .subscribe(muscleGroups => {
        const found = (muscleGroups || []).find(muscleGroup => muscleGroup.id === id);
        if (found) {
          this.muscleGroupData = found;
          this.loading = false;
        } else if (!muscleGroups || muscleGroups.length === 0) {
          if (!dispatched) {
            dispatched = true;
            this.store.dispatch(loadMuscleGroups());
          }
        } else {
          this.muscleGroupData = null;
          this.loading = false;
        }
      });
  }

  openUrl(url: any) {
    window.open(url, '_blank');
  }

  formatDate(dateString: any): any {
    if (!dateString) return '';
    const date = new Date(dateString);
    return this.datePipe.transform(date, 'dd/MM/yyyy');
  }
}
